...
const BASE_EDGES = [
  ...
  railEdge('cloud-controller-manager', 'openshift-ingress-router-guest', 'publishes LB', 'k-purple',
    'Cloud Controller Manager → router LoadBalancer', 'The kubevirt cloud provider realizes the guest router-default Service as the infra-side Apps Ingress LoadBalancer (advertised by MetalLB) — the *.apps north-south path into the guest router. The control-plane API path uses the Shared Ingress Proxy instead.'),
